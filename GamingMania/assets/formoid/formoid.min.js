...
document.body.appendChild(formModalDOM);
...
formComponents.forEach(function (component) {
    ...
    form.addEventListener('submit', function (event) {
        ...
        function parseInput(input) {
            return new Promise(function (resolve, reject) {
                // Validate email
                if (input && input.getAttribute('name') === 'email' && !isValidEmail(input)) {
                    return reject(new Error('Form is not valid'));
                }
        
                // Check if the input element exists and has a name
                if (input && input.hasAttribute('data-form-field')) {
                    var name = input.getAttribute('data-form-field');
                    ...
                    switch (type) {
                        ...
                        case 'checkbox':
                            ...
                            break;
                        case 'radio':
                            resolve(input.checked ? [name, input.value] : null);
                            break;
                        default:
                            resolve([name, input.value]);
                    }
                } else {
                    // Resolve with null for non-existing input or without a name
                    resolve();
                }
            });
        }
